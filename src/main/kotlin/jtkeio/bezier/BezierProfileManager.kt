...
class BezierProfileManager(val field: FRCField) {
    ...
    fun generateBezierProfile(currentPosition: Pair<Double, Double>, targetPosition: Pair<Double, Double>, currentHeading: Double = 180*atan2(targetPosition.second-currentPosition.second,targetPosition.first-currentPosition.first)/PI, targetHeading: Double = 180*atan2(targetPosition.second-currentPosition.second, targetPosition.first-currentPosition.first)/PI): BezierProfile {
        ...
        return BezierProfile(arrayOf(currentPosition, currentGuidePoint, targetGuidePoint, targetPosition)).fitBezierProfileToSelf(3)
    }
    ...
    private fun BezierProfile.fitBezierProfileToSelf(cycles: Int = 1): BezierProfile {
        ...
        for (p in bezierProfileTestPoints.dropLast(10).indices) {
            if (abs(derivativeBezierProfile.calculateIntegralBezierAngle(p / 256.0) - derivativeBezierProfile.calculateIntegralBezierAngle( (p + 10) / 256.0)) > 30) {
                ...
                val closestWaypointIndex = if (waypoints.indexOf(closestWaypoint) == 0) {1} else if (waypoints.indexOf(closestWaypoint) == waypoints.lastIndex) {waypoints.lastIndex-1} else {waypoints.indexOf(closestWaypoint)}
                ...
                val fittedBezierProfile = Array(2) { BezierProfile(Array(waypoints.size) { if (it != closestWaypointIndex) {waypoints[it]} else { waypoints[closestWaypointIndex].first + reflectionLineSlope.first*(0.2*it-0.1) to waypoints[closestWaypointIndex].second + reflectionLineSlope.second*(0.2*it-0.1) }})}.minByOrNull{it.derivativeBezierProfile.calculateIntegralBezierAngle((p+5) / 256.0)}!!.fitBezierProfileToSelf(cycles-1)
                return fittedBezierProfile.fitBezierProfileToSelf(cycles-1)
            }
        }
        return this
    }
